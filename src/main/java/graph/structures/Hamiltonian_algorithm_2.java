        ...
        for (int i = 0; i < node_count.size(); i++) {
            if (node_count.get(i).get(0) > 1) {
                ...
                if(alt_1_2 < alt_3_4){
                    ...
                    int place = 2;
                    if(node_count.get(node_count.get(i).get(4)).get(0) > 1)
                        place = 5;
                    node_count.get(node_count.get(i).get(4)).set(place, node_count.get(i).get(5));

                    //change nodes in counter 2
                    place = 1;
                    if(node_count.get(node_count.get(i).get(5)).get(0) > 1)
                        place = 4;
                    ...
                    //change nodes in counter 1
                    int place = 2;
                    if(node_count.get(node_count.get(i).get(1)).get(0) > 1)
                        place = 5;
                    node_count.get(node_count.get(i).get(1)).set(place, node_count.get(i).get(2));

                    //change nodes in counter 2
                    place = 1;
                    if(node_count.get(node_count.get(i).get(2)).get(0) > 1)
                        place = 4;
